% Performs variable selection via ACDC and lasso on boston housing
% ** DEPRECATED **
...
[p,n] = size(X); 
output = zeros(nv(data),5);

for version = sum(nv(1:data-1))+1:sum(nv(1:data))
    load(['ACDC/ACDC_' num2str(version) '.mat']); 
    output(version-sum(nv(1:data-1)),:) = out;
end

load(['ACDC/LASSO_' num2str(data) '.mat']);
figure(1); set(gca,'FontSize',14);

errorbar(out(:,1),out(:,3),out(:,5),'g--','LineWidth',2); 

hold on
errorbar(output(:,1),output(:,3),output(:,5),'k:','LineWidth',2); 
hold off
...
set(gca,'FontSize',12); 
fg3 = plot(ratioo,betao,'LineWidth',2); 
legend(names{data}); 
...
% to plot two example univariate fits
...
hd = SCAM_Eval(xd,X1(active,:),beta1,h1); 
figure(5);

for d = 1:num
    subplot(2,2,d); 
    set(gca,'FontSize',12);  % ,X1(active(d),:),h1(d,:),'k.'
