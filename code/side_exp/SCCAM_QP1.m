...
%function [beta1,beta2,z1,z2,Ln,obj] = SCCAM_QP1(X,y,lambda,maxit,tol)
[p,n] = size(X); 
...
while iter < maxit && change > tol
    ...
    for d = 1:p
        ...
        res = res + z(d,:);
        %tstart = tic;
        %[beta1(d,:),beta2(d,:),z1(d,:),z2(d,:)] = SCCAM_MOSEK1(res,X(d,:),lambda);
        [beta, z(d,:), c, curLn] = SCAM_MOSEK_quad(res', X(d,:), lambda, L);
        %toc(tstart)
