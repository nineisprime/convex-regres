function [beta1,beta2,z1,z2] = SCCAM_MOSEK_eq(y,x,lambda)
n = length(x); [Xd,ord] = sort(x); dX = Xd(2:n)-Xd(1:n-1);

Hi = [1:2*n 1:n n+1:2*n 2*n+1:4*n-1]; 
Hj = [1:2*n n+1:2*n 1:n 2*n+1:4*n-1];
Hs = [1/n*ones(1,2*n) -1/n*ones(1,n) -1/n*ones(1,n) 10^-6*ones(1,2*n-1)];
H = sparse(Hi,Hj,Hs,4*n-1,4*n-1); f = [-y/n; y/n; zeros(2*n-2,1); lambda];
...
A = sparse(Ai,Aj,As,row,4*n-1); b = zeros(row,1);
...
B = sparse(Bi,Bj,Bs,row,4*n-1); c = zeros(row,1);
