...
while iter < maxit 
    iter = iter + 1;
    for j = 1:p, 
        C(j,:) = Piecewise_Inf(beta(j,:)-1/mu*M(j,:),lambda(j)/mu); 
    end
    ...
    for i = 1:n
        Xi = X - repmat(X(:,i),[1,n]); 
        beta(:,i) = (eye(p)+Xi*Xi')\ ...
                    (C(:,i)+1./mu*M(:,i)+Xi*(h-h(i)-S(:,i)+1/mu*W(:,i)));
        xbi = Xi'*beta(:,i); 
        
        S(:,i) = max(h-h(i)-xbi+1/mu*W(:,i),0); 
        W(:,i) = W(:,i) + mu*(h-h(i)-xbi-S(:,i)); 
    ...
    if mod(iter,50)==0, 
        ...
        disp(['   Iter ' num2str(iter) ' Obj ' ...
            num2str(obj(iter)) ' Score ' num2str(stop_score)]); 
        figure(999); 
        
        subplot(2,1,1); 
        plot(1:n,y,'ko',1:n,h,'r+');
        subplot(2,1,2); 
        plot(max(abs(beta),[],2),'r.'); drawnow;
    end    
end
beta = C;
return
