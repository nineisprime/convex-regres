...
Recently, there has been a flurry of research activity on shape-constrained estimation. \cite{seijo2011nonparametric} and \cite{Lim:12} independently established the consistency of multivariate convex regression, though without an explicit rate. \cite{Cule:10} and \cite{kim2014global} analyzed log-concave density estimation and showed consistency of the MLE with a rate of [TODO:find rate]. Additive shape-constrained modeled were studied by \cite{chen2014generalised} and \cite{pya2014shape}. To the best of our knowledge however, there has been no work on variable selection for shape-constrained estimation. 
