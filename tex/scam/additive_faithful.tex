...
The boundary flatness condition intuitively states that
the conditional effect of $X_k$ weakens as $\mathbf{x}$ approaches the boundary. It is a weak condition because it affects only an $\epsilon$-small region around the boundary; $p(\mathbf{x}_{-j} \given x_j)$ can take arbitrary shapes away from the boundary; it can induce arbitrary correlation structure (provided $p(\mathbf{x}) >0$) and need not be continuous. Before presenting the main theorem, let us see some examples of boundary flat distributions.

\begin{example} The following densities satisfy boundary flatness.
\begin{enumerate}
\item If $\mathbf{p}$ is a product density, then boundary flatness is trivially
satisfied.
\item Let $p(\mathbf{x})$ be a joint density such that $0 < \inf_{\mathbf{x}} p(\mathbf{x}) \leq \sup_{\mathbf{x}} p(\mathbf{x}) < \infty$. Suppose $p(\mathbf{x})$ has a bounded second derivative for all $\mathbf{x}$. If, for all $j$,
$\partial_{x_j} p(x_j,\mathbf{x}_{-j}) =
\partial_{x_j}^2 p(x_j, \mathbf{x}_{-j}) = 0$ at $x_j = 0, 1$. Then it is possible to show that the two derivatives of $p(\mathbf{x}_{-j} \given x_j) = \frac{p(x_j, \mathbf{x}_{-j})}{p(x_j)}$ are 0 at $x_j = 0,1$; boundary flatness thus holds.
\item Suppose $p(\mathbf{x})$ is a bounded density over $[\epsilon, 1-\epsilon]^p$ for some $0 < \epsilon < 1/2$. Let $q(\mathbf{x})$ be a boundary flat density over $[0,1]^p$, take the uniform distribution for instance. Define a mixture $p_{\lambda, \epsilon}(\mathbf{x}) = \lambda q(\mathbf{x}) + (1-\lambda) p(\mathbf{x})$ where $0 < \lambda \leq 1$, then $p_{\lambda, \epsilon}(\mathbf{x})$ is boundary flat over $[0,1]^p$. Furthermore, $\epsilon$ and $\lambda$ can be made arbitrarily small and therefore, any bounded density over a hypercube can be approximated arbitrarily well by a boundary flat distribution.

% $$
% p'(\mathbf{x}_{-j} \given x_j) = \frac{ p(x_j, \mathbf{x}_{-j}) p'(x_j) - 
%    p'(x_j, \mathbf{x}_{-j}) p(x_j)}{ p(x_j)^2 } = 0 
% $$
% \begin{align*}
% p''(\mathbf{x}_{-j} \given x_j) = &
%     \frac{ p(x_j)^2 \left( p(x_j, x_{-j}) p''(x_j) - p''(x_j, x_{-j}) p(x_j) \right) - 2p(x_j)p'(x_j) \left(p(x_j, x_{-j})p'(x_j) - p'(x_j, x_{-j}) p(x_j)  \right) }{p(x_j)^4}
%  \end{align*}
\end{enumerate}
\end{example}
...
Let $p(\mathbf{x})$ be a positive density supported on $C=[0,1]^p$ that satisfies
...
= 0$.  Such a limit condition however is not obeyed by many common distributions such as the multivariate Gaussian distribution. The next example shows that certain convex functions are not additively faithful under certain multivariate Gaussian distributions.
\end{remark}
...
Suppose $p(\mathbf{x})$ is a positive density on $C=[0,1]^p$ satisfying the boundary flatness condition.
