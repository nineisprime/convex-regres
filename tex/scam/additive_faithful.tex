...
Suppose $f$ is a convex and continuously twice differentiable on an open set containing $C$, then $f$ is additively faithful under $p(\mathbf{x})$.
\end{theorem}
...
satisfies the boundary flatness condition. Suppose that $f$ is convex and continuously twice differentiable on an open set around $C$. Let $f^*_k$ and $g^*_k$ be as defined in equations
...
Let $f(\mathbf{x})$ be a convex and continuously twice differentiable function on an open set around $C$. Let $\phi(\mathbf{x}_{-k})$ be a bounded function that does not depend on $x_k$. Then, we have that the unconstrained univariate function
 \begin{align}
