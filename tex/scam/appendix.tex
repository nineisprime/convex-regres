...
We start with a proof of Lemma~\ref{lem:general_int_reduction}. We restate the lemma here for convenience.

\begin{lemma}
Let $F$ be a product distribution on $C=[0,1]^s$ with a positive density function $p$. Let $f: C \rightarrow \R$ be an integrable function. 

Let $\ds f^*_1,...,f^*_s, \mu^* \coloneqq \arg\min \{ \E | f(X) - \sum_{k=1}^s f_k(X_k) - \mu |^2 \,:\, \forall k, \E f_k(X_k) = 0\}$ 
...
Now, we note that as a function of $x_k$, $\E[ f(X) | x_k] - \E f(X)$ has mean zero and we thus finish the proof.

 
 \end{proof}
 
Our reasoning actually proves something stronger. It proves the following lemma:
...
The next lemma adapts Lemma~\ref{lem:general_int_reduction} to convex functions.

\begin{lemma}
\label{lem:int_reduction}
Let $F$ be a product distribution on $C=[0,1]^s$ with positive density function $p$. Let $f: C \rightarrow \R$ be a convex function. 
...
\begin{proof}
The proof is identical to that of Lemma~\ref{lem:general_int_reduction}. We need only verify that $f^*_k(x_k) = \E[ f(X) \given x_k] - \E f(X)$ is convex.

This is true because for every $\mathbf{x}_{-k}$, $f(x_k, \mathbf{x}_{-k})$ is a convex function with respect to $x_k$ and therefore, $\int_{\mathbf{x}_{-k}} p(\mathbf{x}_{-k}) f(x_k, \mathbf{x}_{-k}) d \mathbf{x}_{-k}$ is still convex. 
\end{proof}

 The next proposition, combined with Lemma~\ref{lem:int_reduction}, proves Theorem~\ref{thm:convex_faithful}.
 
\begin{proposition}
\label{prop:faithful_condition}
...
%$\|f_0\|^2_n$ is the average of random variables bounded by $s^2 B^2$ and therefore, with probability at least $1-\frac{C}{n}$, $|\| f_0 \|^2_n - \E f_0(X)^2 | \leq s^2B^2\sqrt{ \frac{1}{cn} \log n}$. Since $\E f_0(X)^2 \leq s^2B^2$, we have that, for all $n > n_0$, with probability at least $1-\frac{C}{n}$, for some constant $c$, $\| f_0 \|_n^2 \leq c s^2 B^2$.
...
%%% Local Variables: ***
%%% mode:latex ***
%%% TeX-master: "icml14/scam_icml.tex" ***
%%% End: ***
