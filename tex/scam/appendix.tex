...
Define $\mathcal{G}$ as $\{ f - f^* + \bar{f}^* \,:\, f \in \mathcal{C}^s_L\}$ as the set of convex additive functions centered around the function $f^* - \bar{f}^*$. 
...
By Corollary~\ref{cor:convexbracket_ln}, there exists a bracketing $\mathcal{G}_\epsilon$ of $\mathcal{G}$ which has sized bounded by [TODO:...], which is an $\epsilon$-bracketing with respect to the $L_2(P)$ norm, and which is also an $(\epsilon + \epsilon_{n,\delta})$-bracketing with respect to the $L_n$ norm with probability at least $1-\delta$.
...
% metric
% function space
Let $\mathcal{C}$ be a set of functions. For a given $\epsilon$ and metric $\rho$ (which we take to be $L_2$ or $L_2(P)$), we define a \textbf{bracketing} of $\mathcal{C}$ to be a set of pairs of functions $\{ (f^L, f^U) \}$ satisfying (1) $\rho( f^L, f^U) \leq \epsilon$ and (2) for any $f \in \mathcal{C}$, there exist a pair $(f^L, f^U)$ where $f^U \geq f \geq f^L$. 
...
It is trivial to extend Kim and Samworth's result to $L_2(P)$ norm for a distribution $P$ with an uniformly upper bounded density.

\begin{corollary}
\label{cor:convexbracket_lp}
Let $P$ be a distribution with a density $p$ upper bounded by $c_u$. Let $\mathcal{C}, b, B, \epsilon_3, K^{**}$ be defined as in Proposition~\ref{prop:convexbracket}. Then,
\[
\log N_{[]} (2\epsilon, \mathcal{C}, L_2(P)) \leq K^{**} \left( \frac{2bB c_u}{\epsilon} \right)^{d/2}
\]
\end{corollary}

Given samples $X_1,..., X_n \in \R^p$, we define $L_n$-norm as $\| f \|^2_n \equiv \frac{1}{n}\sum_{i=1}^n f(X_i)^2$. The following result follows from Corollary~\ref{cor:convexbracket_lp} directly by an union bound. 

\begin{corollary}
\label{cor:convexbracket_ln}
Let $X_1,...,X_n$ be random samples from a distribution $P$. Let $1 > \delta > 0$. Let $\mathcal{C}_\epsilon$ be an $\epsilon$-bracketing of $\mathcal{C}$ with respect to the $L_2(P)$-norm whose size is at most $N_{[]}( 2\epsilon, \mathcal{C}, L_2(P))$. \\

Then, with probability at least $1-\delta$, for all pairs $(f^L, f^U) \in \mathcal{C}_\epsilon$, $\frac{1}{n} \sum_{i=1}^n ( f^L(X_i) - f^U(X_i)^2 \leq \epsilon + \epsilon_{n, \delta}$, where 
$\epsilon_{n,\delta} \equiv 
B^2 \sqrt{ \frac{ \log N_{[]}(2\epsilon, \mathcal{C}, L_2(P)) + \log \frac{1}{\delta}}{2n}} = 
B^2 \sqrt{ \frac{K^{**} (2bBc_u)^{d/2} + \log \frac{1}{\delta}}{2\epsilon^{d/2} n} }$
\end{corollary}

\begin{proof}
$(f^L(X_i) - f^U(X_i))^2$ is at most $B^2$. There are $N_{[]}(2\epsilon, \mathcal{C}, L_2(P))$ pairs $(f^L, f^U)$. The corollary follows from a direct application of union bound and Hoeffding Inequality.
\end{proof}
