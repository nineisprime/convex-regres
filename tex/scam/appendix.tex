...
\]
then we easily see that the first coordinate of $\mathbf{u}_k$ can be set to some value in $(-1,1)$ and we can set $\mu_{k,i} > 0$ for $i=2,...,n$. 
...
\hat{R}_s (f') &= \| f_0 + w - \bar{f}_0\mathbf{1}_n - \bar{w} \mathbf{1}_n - f' \|_n^2\\
     &= \| f_0  - \bar{f}_0\mathbf{1}_n - \bar{w}\mathbf{1}_n - f'\|_n^2 
       + 2 \langle w, f_0 - \bar{f}_0\mathbf{1}_n -\bar{w}\mathbf{1}_n - f'\rangle_n 
       + \| w \|_n^2 \\
