...
\[
min_{f_k} \{ \E( f(X) - \E f(X) - \sum_{k'\neq k} f_{k'}(X_{k'}) - f_k (X_k))^2 \,:\, \E f_k(X_k) = 0 \}
\]
...
For a function $g : \R^s \rightarrow \R$, define $\hat{R}_s(g) \coloneqq \| f_0 + w - \bar{f_0 + w} - g \|_n^2$ as the objective of the \emph{restricted} regression and define $R_s(g) \coloneqq \E | f_0(X)  + w - \mu - g(X) |^2$ as the population risk, where $\bar{f_0 + w} = \frac{1}{n} \sum_i f_0(X^{(i)}) + w_i $ and $\mu = \E f_0(X)$. Note that we \emph{subtract} out the mean in the two risk definitions.
...
Now that we bounded the cost of approximating $f^{*s}$ with the empirically centered $f^{*s} - \bar{f}^{*s}$, we move on to the proof of the main result.
