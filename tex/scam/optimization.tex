...
\trm{such that}\;\;  & d_{\perm{2}k}, \ldots , d_{\perm{n-1}k} \geq 0  	
               \qquad \trm{(convexity).} \nonumber 
\end{align}
Likewise, suppose $\{\hat{d}_k\}_{k=1,...p}$ is a solution to \eqref{opt:alternate_opt}, define $\hat{\beta}_{\pi_k(i)k} = \sum_{j\leq i} \hat{d}_{\pi_k(j)k}$ and $\hat{f}_k = \bar{\Delta}_k \hat{d}_k$. Then $\{\hat{f}_k, \hat{\beta}_k\}_{k=1,...,p}$ is an optimal solution to \eqref{np}. $\bar{\Delta}$ is a $n$ by $n-1$ matrix defined by \eqref{eq:deltabar_defn}.
