...
After optimization, the function estimate for an input vector $\bds{x}$ is, according to (\ref{hyper}),
\begin{equation}
\begin{split}
...
d_{\perm{n-1}k})$, and $\mu_k = f_k(x_{\perm{1}k})$.
...
\trm{s.t. }  & d_{\perm{2}k}, \ldots , d_{\perm{n-1}k} \geq 0  	
               \qquad \trm{(convexity)} \nonumber 
\end{align}
...
\textrm{for all}&\textrm{ $k$ such that $\hat{d}_k = 0$: }  \nonumber \\
 ...
 \trm{s.t.} & c_{\perm{2}k}, \ldots, c_{\perm{n-1}k} \leq 0 \qquad \trm{(concavity)} \nonumber
\end{align}
