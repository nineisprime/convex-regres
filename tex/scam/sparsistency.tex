...
\in \{1,...,p\}$, Let $\Delta_{k, j}$ be the $j$-th column of $\Delta_k$, i.e. the $n$-dimensional vector $\max( X_k - X_{k (j)} \mathbf{1}, 0)$. \\
...
\begin{packed_enum}
\item Let $\mathcal{X} = [-b, b]^p$. Let $F$ be a distribution supported and positive on $\mathcal{X}$. Let $X^{(1)},..., X^{(n)} $ be iid samples from $F$.
\item Let $Y = f_0(X) + \epsilon$ where $f_0$ is the true function and $\epsilon$ is noise. Let $Y^{(1)},...,Y^{(n)}$ be iid samples.
\item Let $s \leq p$ be a positive integer. Let $S = \{1,...,s\}$ denote the relevant variables, i.e.,
  $f_0(X) = f_0(X_S)$.
\item Let $f_{01},...,f_{0s} \coloneqq \argmin_{f_1,...,f_s} \{ \E(f_0(X) - \sum_{k=1}^s f_k(X_k))^2 \,|\, \E[f_k(X_k)] = 0 \}$ where $\mu = \E f(X)$.
\end{packed_enum}
