...
Let $\pi_k(i)$ be a reordering of $X_k$ in ascending order so that $X_{k \pi_k(n)}$ is the largest entry. Let $\mathbf{1}_{\pi_k(i:n)}$ be 1 on the coordinates $\pi_k(i),\pi_k(i+1),...,\pi_k(n)$ and 0 elsewhere. Assume without loss of generality that $X_{k\pi_k(n)} - X_{k\pi_k(1)} \geq 1$.

Suppose for all $k\in S^c$, for all $i=1,\ldots,n$, $\lambda_n > (X_{k\pi_k(n)} - X_{k\pi_k(1)})| \frac{32}{n} \hat{r}^\tran \mathbf{1}_{\pi_k(i:n)}|$ and $\max_{i=1,...,n-1} \frac{X_{k\pi_k(i+1)} - X_{k\pi_k(i)}}{X_{k\pi_k(n)} - X_{k\pi_k(1)}} \geq \frac{1}{16}$.
...
a distribution $F$ with a density $p(\mathbf{x})$ that is supported and strictly positive on $\mathcal{X}=[-b,b]^p$.
...
\big)^2.
\end{equation}
$f^*$ is the unique minimizer by Theorem~\ref{thm:acdc_faithful}. Similarly, we define $\mh \mathcal{C}^1$ as the set of univariate concave functions supported on $[-b, b]$ and define
\begin{equation}
...
Suppose assumptions A1-A5 hold. Define $\tilde{\sigma} \equiv \max(\sigma, B)$. Suppose $p \leq O\big( \exp(c n) \big)$ and $n \geq C$ for some positive constants $c, C$ dependent on $b$.  Suppose also
\begin{equation}
\lambda_n \geq 512 b s \tilde{\sigma}  \sqrt{ \frac{\log^2 np}{n}}.
\end{equation}  
Then with probability at least $ 1 - \frac{24}{n}$, for all $k \in
S^c$, for all $i=1,\ldots,n$,
\begin{equation}
\lambda_n \geq (X_{k\pi_k(n)} - X_{k \pi_k(1)}) \Big| \frac{32}{n}\hat{r}^\tran \mathbf{1}_{(i':n)_k} \Big| ,
\end{equation}
$\max_{i'} \frac{X_{k\pi_k(i'+1)} - X_{k \pi_k(i')}}{X_{k\pi_k(n)} - X_{k\pi_k(1)}} \leq \frac{1}{16}$, and both the AC solution $\hat{f}_k$ from optimization~\eqref{opt:alternate_opt} and the DC solution $\hat{g}_k$ from optimization~\eqref{opt:alternate_opt_concave} are zero. 
\end{theorem}
...
B)$.  Suppose 
\begin{equation}
\lambda_n \leq 512 b s \tilde{\sigma} \sqrt{\frac{\log^2 np}{n}}
\end{equation}
