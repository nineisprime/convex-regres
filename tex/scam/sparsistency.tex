...
\begin{enumerate}
\item The data $X^{(1)},..., X^{(n)} \sim F$ are iid from
...
$f_0(X) = f_0(X_{S_0})$, where $S_0 = \{1,...,s_0\}$ is the set of
relevant variables.
\end{enumerate}
...
\item[A4:] $W$ is mean-zero sub-Gaussian, independent of $X$, with sub-Gaussian scale $\sigma$, i.e. for all $t \in \R$, $\E e^{t \epsilon} \leq e^{\sigma^2 t^2 / 2}$.
\end{packed_enum}
