...
Suppose assumptions A1-A5 hold. Define $\tilde{\sigma} \equiv \max(\sigma, B)$ and define $\mathsf{range}_k = X_{\pi_k(n)k} - X_{\pi_k(1)k}$. Suppose $p \leq O\big( \exp(c n) \big)$ and $n \geq C$ for some positive constants $0 < c <1$ and $C$.  Suppose also
\begin{equation}
