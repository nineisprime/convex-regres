...
We have added a new Section 3.2 to the paper to further discuss the boundary flatness condition as well as giving more examples. The following paragraph has been added to briefly discuss extending the same additive faithfulness analysis to densities whose support is not a hypercube:
...
The original paper also briefly discussed extending boundary flatness to densities with an unbounded support.

The key ideas behind our analysis do not depend critically on the fact that the support is a hypercube. The hypercube assumption is made for convenience, to minimize technical details.
