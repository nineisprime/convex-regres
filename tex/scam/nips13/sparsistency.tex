...
technique''~\cite{Wain:09a}. Primal-dual witness explicitly solves all the dual variables. Because our optimization is more complex, we do not solve the dual variables on $S$, we instead write the dual variables on $S^c$ as a function of the restricted regression \emph{residual}, which is implicitly a function of the dual variables on $S$.
