...
[p,n] = size(X); dX = zeros(p,n-1); ord = zeros(p,n); Ln = zeros(p,maxit);
...
for d = 1:p, B1(ord(d,:),:) = B; B1(:,ord(d,:)) = B1; B2 = B2+1/(mu*n)*B1; end; B2 = inv(B2);
rg = (0:n-2)*(n-1)+(1:n-1); ndx = (ord-1)*p + repmat((1:p)',[1 n]);
...
while iter < maxit
    ...
    h = zeros(p,n); eta = 1/(mu*n)*repmat(y',[p 1])-1/mu*repmat(q,[1 n]);
    resp = dX.*beta-1/mu*M; eta(ndx) = eta(ndx) + [zeros(p,1) resp] - [resp zeros(p,1)];
    h(ndx) = eta(ndx)*B; vec = B2*(sum(h,1)'); h(ndx) = h(ndx) - 1/(mu*n)*vec(ord)*B;
    
    dh = h(ndx2) - h(ndx1);
    gd = C+1/mu*O + [zeros(p,1) S-1/mu*W]-[S-1/mu*W zeros(p,1)] + dX.*(dh+1/mu*M);
    for d = 1:p, Gd = G; Gd(rg) = Gd(rg) + dX(d,:).^2; beta(d,:) = Gd\(gd(d,:)'); end
    dbeta = beta(:,2:n-1) - beta(:,1:n-2);
    S = max(dbeta+1/mu*W,0); W = W + mu*(dbeta-S); M = M + mu*(dh - dX.*beta);
    
    q = q + mu*sum(h,2); O = O + mu*(C-beta);
    Ln = max(abs(C),[],2); obj(iter) = 0.5/n*sum((y-sum(h,1)').^2) + lambda*sum(Ln);
    Ln1 = max(abs(beta),[],2);
    if mod(iter,100)==0
...
if b(n) < 0
    ...
    z(ndx(1:pos)) = sign(x(ndx(1:pos))).*zinf; z(ndx(pos+1:n)) = x(ndx(pos+1:n));
end
return
...
n = 1000; p = 100;
k = 5; lambda = 3*sqrt(log(n*p)/n); maxit = 10; tol = 10^-6; 
J = ones(p,1)==0; Q = eye(k); X = randn(p,n); ord = randperm(p); J(ord(1:k)) = 1==1;
...
figure(1); subplot(2,1,1); plot(t1/iter1*(1:iter1),obj1,'r.-',t2/iter2*(1:iter2),obj2,'k.-');
xlabel('Elapsed time (seconds)'); ylabel('Objective value'); legend('QP','ADMM');
subplot(2,1,2); plot(1:p,Ln1,'r.',1:p,Ln2,'k.'); 
xlabel('Feature dimension'); ylabel('|\beta|_\infty'); legend('QP','ADMM');