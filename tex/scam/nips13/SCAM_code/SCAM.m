...
data = sum(version>cumsum(nv)) + 1; lambda = lambda0{data}(version - sum(nv(1:data-1)));
switch data
...
[p,n] = size(X); mrun = 10; b = 5; m = fix(n/b); % 5-fold CV
err = zeros(2,mrun); num = zeros(1,mrun);
for run = 1:mrun
    ord = randperm(n);
    for i = 1:b
        ndx = ones(n,1)==0;
        if i<b, ndx(ord((i-1)*m+1:i*m))=1==1; else ndx(ord((i-1)*m+1:end))=1==1; end
        [erri,numi] = SCAM_Main(X,y,ndx,lambda);
        err(:,run) = err(:,run) + erri./[b-1; 1]; num(run) = num(run) + numi/b;
...
[p,n] = size(X); [X1,X2] = SCAM_Unit(X,ndx);
y1 = y(~ndx); ym = mean(y1); y2 = y(ndx); maxit = 20; tol = 10^-6;
...
yo = zeros(n,1); yo(~ndx) = sum(h1,1)'+ym; yo(ndx) = sum(h2,1)'+ym;
